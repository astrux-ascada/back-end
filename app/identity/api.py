...
from app.identity.schemas import UserCreate, UserLogin, TokenWithUser, UserRead, TfaToken
...
def verify_tfa_token(token_data: TfaToken, current_user: User = Depends(get_current_active_user), auth_service: AuthService = Depends(get_auth_service)):
    """Verifica un token 2FA para autorizar una operación crítica."""
...
@router.post("/sessions/clear-all", dependencies=[Depends(get_current_superuser)])
