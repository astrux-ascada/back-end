...
asset_hierarchy = Table(
    "asset_hierarchy",
    Base.metadata,
    Column("parent_asset_id", Integer, ForeignKey("assets.id", ondelete="CASCADE"), primary_key=True),
    Column("child_asset_id", Integer, ForeignKey("assets.id", ondelete="CASCADE"), primary_key=True),
)
...
class Asset(Base):
    ...
    asset_metadata = Column(JSONB) # RENAMED from 'metadata'
